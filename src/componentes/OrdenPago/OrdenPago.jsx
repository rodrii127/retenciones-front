...
export const OrdenPago = (props) => {
      ...
      const [selectedDateOrdenDePago, handleDateChangeOrdenPago] = useState(
            new Date()
      );
      ...
      const getInvoiceUri = (parameters) => {
            ...
            const providerName = document.querySelector(
                  ".busqueda_factura .criterio_busqueda .input_buscador .caja_contenedor input"
            ).value;
            ...
            if (providerName) {
                  invoiceUriWithParams = invoiceUriWithParams
                        .concat(providerParam)
                        .concat(providerId);
            }
            ...
            fetch(getInvoiceUri(getParameters()), {
                  ...
                  .then((res) => {
                        if (res.length === 0) {
                              mensajeArriba(
                                    "info",
                                    "No hay facturas en esas fechas..."
                              );
                        } else {
      ...
      const createPayOrder = () => {
            const selectedItems = selectedCheckboxItems.map((elemento) => {
                  return elemento.id;
            });
            ...
            fetch(payOrderUri, {
                  ...
                  .then((res) => {
                        mensajeArriba(
                              "success",
                              "Orden de Pago creada con éxito"
                        );
                        createPayOrderPDF(
                              res.id,
